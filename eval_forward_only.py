...
def collate_batch(batch):
    # Follow the same flattening logic used in `train_efficient.py` collate_fn.
...
def main():
    ...
    parser.add_argument("--model-dir", type=str, default="./vllfl_adapters_efficient", help="Directory with saved model weights (from training)")
    parser.add_argument("--processor", type=str, default="google/owlv2-base-patch16-ensemble", help="Processor to use for preprocessing (model dir may not contain processor files)")
    ...
    # Load model from fine-tuned directory if provided; processor from base checkpoint (saved adapters often don't include processor files)
    model = Owlv2ForObjectDetection.from_pretrained(args.model_dir)
    processor = Owlv2Processor.from_pretrained(args.processor)
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(loader):
            ...
            # DEBUG: print shapes/types for attention_mask troubleshooting
            try:
                print({k: (v.shape, v.dtype, type(v)) for k, v in inputs.items()})
            except Exception:
                pass
